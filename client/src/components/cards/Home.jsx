...
function Home() {
    ...
    useEffect(() => {
        const fetchClubs = async () => {
            try {
                const response = await axios.get('/api/bookclubs');  // replace with your actual API endpoint
                const data = await response.data;
                console.log(data);
                setFilteredClubs(data);  // Update the filteredClubs state
            } catch (error) {
                console.error("Failed to fetch book clubs:", error);
            } 
        };

        // Only fetch when clubs change
        fetchClubs();
    }, [clubs]);  // This effect runs when the clubs data changes
